#!/usr/bin/env python3
"""Report how hard the book's prose is to read.

This is a **report, not a gate**. It is deliberately not wired into the test
suite beside `check_prose.R`, because that guard checks facts that are either
true or false about a line, while sentence length is a judgment. A threshold
here would be enforced by splitting sentences in half rather than by rewriting
them, which is the metric improving while the prose gets worse.
...
pass "reads more simply now" when the numbers say it does not.
    ...
    python3 book/readability.py joins      # one chapter, with its long sentences

What it measures, and what it cannot. Words per sentence and the share of long
sentences are the two things that correlate with difficulty for a reader working
in a second language, and both are countable. The things that matter just as
much, meaning idiom, metaphor, a term used before it is defined, or the point
buried after a dash, are not countable, and a good score does not mean the prose
is plain. Read the long sentences the report prints; that list is the useful half
of the output.

Prose only: code chunks, tables, headings, list bullets and YAML are stripped,
because a bullet fragment is not a sentence and a table cell is not prose.
"""
...
TARGET = 18.0   # 15 to 18 words, one idea per sentence
...
def sentences(text):
    """Split into sentences. A colon before a code chunk ends one, which is the
    book's most common shape and the one a naive splitter runs through."""
    out = []
...
def chapters():
    for path in sorted(glob(os.path.join(BOOK, "**", "*.qmd"), recursive=True)):
        ...
        # and `_freeze/` are output, and a `_`-prefixed file is one the author
...
def main():
    ...
    if not rows:
        print("no chapter long enough to measure yet.")
        return
    ...
    print(f"\nTarget is {TARGET:.0f} words per sentence. "
