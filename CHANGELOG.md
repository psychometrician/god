...
come first, so a reader who stops after Part I can answer real questions about a
table; reshaping is next; and everything after that adds one idea at a time.

Three appendices are new. The two languages, which was a chapter. Every word in
the grammar on one page, generated from the engine's own vocabulary. And the same
sentence written out in dplyr, pandas, polars, PySpark and SQL, generated by
`show_as`, for anyone arriving from one of them.
