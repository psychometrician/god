...
fn a_pipeline_survives_being_written_back_out() {
    let pipelines = [
        ...
        r#"sales then add [name] as look_up([region], "W", "West", "E", "East", otherwise [region])"#,
        r#"sales then add [flag] as look_up([revenue], 0, "none", otherwise "some")"#,
        r#"sales then add [cleaned] as look_up([region], "", missing, otherwise [region])"#,
